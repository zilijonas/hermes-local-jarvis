...
(function boot() {
  // Harness/debug hook (see ui/dev-harness.html): lets the 2D intelligence
  // core be created and driven directly, and the App mounted with a stubbed
  // SDK, without the Hermes host. Exposed unconditionally (before the SDK
  // guard) so the harness works from file://.
  window.__JARVIS_VOICE_INTERNALS__ = { createVisualizer: createVisualizer, App: App };
